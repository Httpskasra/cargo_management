...
از منوی «آمار و گزارش» می‌توانید گزارش را با فیلترهای تاریخ، راکب، نوع رانشیت و وضعیت بررسی کنید. این صفحه شامل KPI، نمودار وضعیت، روند روزانه، تفکیک NDX/SAPA Post و عملکرد راکب‌هاست.

همچنین در صفحه «جستجوی پیشرفته» بعد از هر جستجو، گزینه «خروجی اکسل همین جستجو» همان نتایج فیلترشده را به فایل Excel-compatible با پسوند `.xls` تبدیل می‌کند.
