...
export async function GET() {
  ...
  const [today, totalReturned, delivered, inTransit, activeRiders, runsheets] = await Promise.all([
    ...
    prisma.runsheetItem.count({where:{status:'DELIVERED',registeredAt:{gte:start}}}),
    prisma.runsheetItem.count({where:{status:{in:['IN_TRANSIT','NORMAL']},registeredAt:{gte:start}}}),
