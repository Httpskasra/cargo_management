...
export async function PATCH(req:NextRequest){
 ...
 const status=normalizeStatus(String(b.status||'')),result=await prisma.runsheetItem.updateMany({where:{id:{in:ids}},data:statusData(status)});ids.forEach(id=>broadcast('item.updated',{id}));return NextResponse.json({count:result.count,status})
}
export async function DELETE(req:NextRequest){
 const b=await req.json(),ids:number[]=Array.isArray(b.ids)?b.ids.map(Number).filter(Boolean):[Number(b.id)].filter(Boolean);if(!ids.length)return NextResponse.json({error:'حداقل یک مرسوله را انتخاب کنید'},{status:400})
 const items=await prisma.runsheetItem.findMany({where:{id:{in:ids}},select:{id:true,runsheetId:true}});const result=await prisma.runsheetItem.deleteMany({where:{id:{in:ids}}});items.forEach(item=>broadcast('item.deleted',item));return NextResponse.json({count:result.count})
}
